  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm">
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          <div className="bg-slate-50 rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-slate-900">{company.total_product_jobs}</div>
            <div className="text-slate-500 text-sm">Total Jobs</div>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-blue-600">{filteredJobs.length}</div>
            <div className="text-slate-500 text-sm">{usOnly ? "US Jobs" : "Showing All"}</div>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 text-center">
            ...
            <div className="text-slate-500 text-sm">Last Checked</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer select-none bg-white border border-slate-200 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors">
          <input
            type="checkbox"
            checked={usOnly}
          ...
          {Array.from(jobsByDate.entries()).map(([date, jobs]) => (
            <div key={date}>
              ...
              <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
                {jobs.map((job) => (
                  <div key={job.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      {!job.is_baseline && (
                        <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-xs font-semibold shrink-0">
                          NEW
                        </span>
                      )}
                      <span className="text-slate-900 font-medium truncate">{job.job_title}</span>
                    </div>
                    {job.job_location && (
                      <span className="text-slate-500 text-sm shrink-0 max-w-[250px] truncate flex items-center gap-1">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-700 text-sm font-medium shrink-0 flex items-center gap-1 hover:underline"
                    >
                      View
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
