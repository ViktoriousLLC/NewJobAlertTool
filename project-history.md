...
## 2026-05-31 — Auto-scrape on subscribe: make "add a company" actually populate (DEV-54)

**Context.** A design-review question from the user exposed a real gap: the on-demand scraper (DEV-52, #139) existed but nothing *triggered* it on a normal add. Single user-submitted companies already scraped on add (via `POST /api/companies`), but the new bulk add-by-category flow and catalog subscribes did not, so a freshly-added catalog company showed nothing until the next 14:00 cron. The user's framing: "it's on-demand, so we use it when I add something, right?" — correct intent, not yet wired.
...
**What shipped.**
- **`POST /api/webhooks/resend`** (`backend/src/routes/resendWebhook.ts`) — NO JWT (it's server-to-server); authenticated by the **Svix signature** Resend sends. Handles `email.sent / delivered / opened / clicked / bounced / complained`, forwarding each to PostHog as `email_sent / email_delivered / email_opened / email_clicked / email_bounced / email_complained` with `{ email_id, subject, tags, recipient_domain }` (+ `link` on clicks, read from `data.click.link` — `email.opened` has no nested object). Never throws on a single event: once the signature is valid it always 200s (so Resend doesn't retry-storm over a downstream hiccup); a bad event is logged + Sentry'd and skipped.
- **Signature verification** (`backend/src/lib/svixVerify.ts`) — implemented the documented Svix scheme directly (HMAC-SHA256 of `${svix-id}.${svix-timestamp}.${rawBody}`, key = base64 after the `whsec_` prefix, constant-time compare against each `v1` entry, ±5 min replay tolerance) rather than add the `svix` npm dependency. Self-contained, no supply-chain addition, mirrors the existing `safeCompareSecret()` constant-time pattern. New env var **`RESEND_WEBHOOK_SECRET`**; unverified requests 401 and a missing secret fails closed.
- **Person-stitching** (`backend/src/lib/hashEmail.ts`) — the distinctId is `hashEmail(recipient)`, SHA-256 hex of `email.toLowerCase().trim()`, **byte-for-byte identical** to the frontend `hashEmail` that `identify()`s the logged-in user. So an `email_opened` event attaches to the user's existing PostHog person, not a new orphan. The raw address is never sent as a property (PII) — only the hash (as distinctId) and the `recipient_domain`.

**The one real subtlety: raw body vs. the global JSON parser.** Svix signs the exact bytes Resend sent; `express.json()` would parse + re-serialize the body (changing whitespace/key order) so the HMAC would never match. The route is mounted with `express.raw({ type: "application/json" })` **before** the global `express.json()` and before the `/api/` rate limiter — so `req.body` is a Buffer for this one path, every other route still gets parsed JSON, the 256kb global JSON limit is untouched, and a legitimate burst of deliveries isn't 429'd (the route is signature-gated, not auth-gated).
...
**Why staging at all.** The user kept hitting the same wall: he can't test authenticated features on Vercel previews because everything past login needs a real session, and the rejected hack was to loosen *prod* auth redirect URLs to make previews log in. The standard answer is a real staging environment, greenlit as DEV-64.

**Architecture decision — separate Supabase project, NOT same-project staging tables.** The user's first instinct was "add a few `staging_` tables to the prod project." Researched the three options (separate project / Supabase Branching / same-project tables) against public docs before building. Rejected same-project tables on a hard constraint: `auth.users` is a single per-project system schema, so a staging signup would create a REAL prod user — it's structurally impossible to isolate auth that way, which defeats the whole point. Rejected Branching: it needs the Pro plan (~$25+/mo) and starts empty anyway (still needs a seed). Chose a **separate Supabase project** — the only option with full auth/RLS/data isolation, it's Supabase's own documented pattern, and it's **$0** (free tier allows 2 projects per org; confirmed via the cost API). Created `NewJobAlertTool-STAGING` (`pcrurbrppundzfbcjxrg`, us-east-1).

**Schema replication.** Prod's base tables predate its tracked migrations (project began Feb 2; earliest migration is May 8), so replaying migration files would not rebuild the schema. Instead introspected prod directly and rebuilt all 17 public tables + 124 columns + 16 FKs + 51 indexes + 18 RLS policies + the one function on staging, schema-only. Verified parity with an identical md5 signature over 231 schema-signature rows on both projects — exact structural copy, zero data rows.

**Catalog-clone script (`backend/tools/seed-staging.mjs`, `npm run seed:staging`).** Copies ONLY the catalog tables — `companies`, `seen_jobs`, `comp_cache` — prod → staging, DB-to-DB via service-role clients so no row data passes through any terminal/log/chat. Explicitly never touches user/PII tables (those stay synthetic in staging). Idempotent (upsert on id), optional `WIPE=1` for a pristine refresh, and a hard guard that refuses to run unless the target is the known staging ref and differs from prod — the clone can never write to prod. Run on-demand per feature or piggybacked on the daily scrape.
...
## 2026-06-01 — Weekly digest: UTM tagging + a Monday "Product Pulse" metrics block (DEV-65)

The metrics work (admin dashboard, Resend webhook) made the weekly digest the natural place to (a) make our own outbound links attributable and (b) put the product numbers Vik wants weekly in front of him without opening a dashboard.

**Part 1 — UTM tagging so PostHog can attribute the digest's traffic.** Added a small `withUtm(url, source)` helper in `weeklyDigest.ts` and applied it to every link that points at OUR site (`FRONTEND_URL`, default `https://newpmjobs.com`). LinkedIn-post links get `utm_source=linkedin&utm_medium=social&utm_campaign=weekly-digest`; email links get `utm_source=email&utm_medium=email&utm_campaign=weekly-digest`. The helper merges correctly with an existing query string (`?` vs `&`) and preserves a trailing `#fragment`. External job/career links are deliberately left untouched — they leave our domain, so tagging them would be noise. This directly closes the gap the referrers insight flagged in its own description ("utm_source is not captured, so a UTM breakdown isn't possible"): now the digest's clicks are tagged at the source. The post's closing CTA became a full UTM'd URL (LinkedIn auto-links and preserves it); the email gained one own-site CTA link ("See this week's roles on the live feed").

**Part 2 — "Product Pulse", a Monday-only metrics section (`backend/src/jobs/productPulse.ts`).** Two tiers, designed so the metrics can never break the send:
- **Supabase tier, always on (no key required):** new signups in the last 7 days + total users (from `listAllUsers().created_at` — the paginated helper, not the perPage=50 footgun), total active `user_subscriptions`, count of `is_active` companies. Straight from our own DB via `head+count`.
- **PostHog tier, only when `POSTHOG_PERSONAL_API_KEY` is set:** weekly active logged-in users (HogQL over the authenticated events), signup-funnel conversion %, conversion by device, top 3 referrers, top nav-button clicks, companies-tracked-per-user split. Read from the four dashboard insights built earlier this DEV-65 (`GET /insights/?short_id=` returns the cached result, so the Monday email spends ~no PostHog compute) plus one small HogQL `/query/` call for WAU. Each slice is independently `Promise.allSettled`-wrapped, so one failing insight can't blank the others.
- **Graceful fallback:** key unset → only the Supabase metrics render plus a "Full dashboard ->" link. `computeProductPulse()` catches its own errors and the renderer emits a short "metrics unavailable" note rather than throwing — a metrics failure is structurally incapable of breaking the digest.

**Design choices worth recording.** (1) The PostHog *read* key is a different credential from the publishable `POSTHOG_API_KEY` we already use for *capture* — kept them separate (and used a separate `POSTHOG_HOST_QUERY` default of `us.posthog.com` vs the ingest `us.i.posthog.com`) so a future env edit can't cross-wire write-capture with read-query. (2) Reading the pre-built insights instead of re-deriving everything in HogQL means the email shows exactly the numbers the dashboard shows, and stays cheap. (3) `renderEmailHtml` got an optional `productPulse` arg (defaults null) — Monday `sendWeeklyDigest` computes + passes it; the preview route force-computes it so the block is reviewable any day; everything else (non-Monday, other callers) is unchanged and backward-compatible.

**Owner action to turn on the richer tier:** provision a PostHog **Personal API key** with scopes **`insight:read` + `query:read`** on project 311721 and set it as `POSTHOG_PERSONAL_API_KEY` on Railway. Until then the digest still ships the core Supabase metrics + the dashboard link. `tsc --noEmit` clean; `withUtm` query-string/fragment cases and all three render states (key-on / key-off / DB-failure) verified locally.

---

## 2026-06-01 — Daily email LATE (not lost): 519-company run + uncapped concurrent Puppeteer (DEV-62)

**Symptom:** the daily alert email hadn't arrived by the usual time (second consecutive day the pipeline missed its timing), plus two Sentry `error`s on `POST /api/subscriptions`: "Failed to launch the browser process! … pthread_create: Resource temporarily unavailable" (Ticketmaster, 14:44:42 UTC) and "socket hang up" (Modern Treasury, 14:46:08).

**What actually happened — NOT a repeat of 2026-05-31.** A five-stream investigation (Sentry + Railway logs + code + `cron_runs`/`email_send_log`) showed the 14:00 run started clean at 14:01 and was STILL scraping at 15:04 ("Found 10 product jobs for Twitch"), 361/519 done — **healthy and progressing, just slow.** Zero SIGTERM / OOM-kill / container-restart; the active deploy was from 11:41 (pre-window), so no merge-redeploy hit it. The catalog doubling (247 → 519) stretched the strictly-sequential run (one company at a time + a 5s delay) to ~80+ min, and the email step runs only AFTER the loop — so the alert lands ~15:25-15:35 instead of near the start of the window. **The email was late, not lost.** (2026-05-31 was a process-kill; this is a long-run.)

**The real defect the Sentry noise exposed:** there is no global cap on concurrent Chromium launches anywhere in the backend. The only guard, `dailyCheckRunning`, was module-private and guarded re-entrant *daily* runs only — it did NOT guard `scrapeCompaniesByIds`, which `POST /api/subscriptions` fires fire-and-forget (DEV-54). So every user who subscribes during the ~80-min run starts an independent scrape loop launching its OWN Chromium alongside the daily loop. N concurrent subscribes = N+1 Chromium process trees competing for the container's PID/thread/memory budget → a fresh browser can't fork (`pthread_create EAGAIN`, GPU/zygote/network-service crashes). Both errors were from the subscribe path; ZERO from the cron's own code. Contained today (handled exceptions, no process death) but a latent risk of OOM-killing the run before the email step.

**Fix (this PR, code-only, no migration):** gate the subscribe-scrape. `scrapeCompaniesByIds` now no-ops when the daily run is in flight OR another subscribe-scrape is already running (a `subscribeScrapeActive` single-flight latch + the newly-exported `isDailyCheckRunning()`); the subscribe route also checks `!isDailyCheckRunning()` before firing. Stays fire-and-forget (never blocks the subscribe response under Cloudflare's ~100s timeout). The daily cron is the guaranteed backstop for these companies, so skipping is safe + self-healing. **Merged AFTER 16:00 UTC** — never during the 13:55-16:00 cron window (the cron-window-guard blocks it anyway, and a merge-redeploy mid-run is exactly the 2026-05-31 failure mode).

**Structural follow-ups (logged, not in this PR):** (1) DEV-58 worker-cutover so the daily run lives off the web service and web traffic physically can't contend with it (also closes the 2026-05-31 hole); (2) a real global Puppeteer concurrency semaphore that EVERY launch path goes through — this PR is the minimal version; (3) refresh the run-length expectation + cron window: 519 companies pushes the email deep into the window, and a future catalog doubling needs bounded-parallel scraping (3-5 at a time) so time-to-email doesn't keep doubling.

**User impact:** low — late (~60-90 min), not deprived; jobs don't expire in 90 min, so no user notice is warranted *if it sent late*. Only a missed send (run OOM-dies before the email step) would warrant a SILENT recovery via `POST /api/cron/email-only`. Process severity stays high (2nd consecutive day the pipeline missed timing, via a different mechanism each time — DEV-62 silent-failure class: a growable resource not bounded at the runtime chokepoint). Whether the email actually sent is being confirmed via `email_send_log`.
