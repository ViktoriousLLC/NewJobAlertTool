...
**Catalog-clone script (`backend/scripts/seed-staging.mjs`, `npm run seed:staging`).** Copies ONLY the catalog tables — `companies`, `seen_jobs`, `comp_cache` — prod → staging, DB-to-DB via service-role clients so no row data passes through any terminal/log/chat. Explicitly never touches user/PII tables (those stay synthetic in staging). Idempotent (upsert on id), optional `WIPE=1` for a pristine refresh, and a hard guard that refuses to run unless the target is the known staging ref and differs from prod — the clone can never write to prod. Run on-demand per feature or piggybacked on the daily scrape.
