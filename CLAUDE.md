...
| **Phenom** (added 2026-05-14) | Server-rendered DDO: `phApp.ddo["eagerLoadRefineSearch"]` | eBay (`jobs.ebayinc.com`, tenant EBAEBAUS). Generic `scrapePhenomCareers(baseDomain, label, stats)` — future Phenom companies need only a DB row, no code. **10-job server-render cap** (all further pagination is client-side Vue). |
...
| Puppeteer | HTML scraping (120s timeout) | Google, Ametek, fallback for unknown |
