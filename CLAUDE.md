...
**Known monitoring gap — silent 0-PM results (diagnosed 2026-05-14, fix planned):** A scraper returning `[]` with HTTP 200 currently marks `last_check_status = "success"`. This is structurally indistinguishable from "company genuinely has 0 PMs" without a second source. Eyeball audit on 2026-05-14 found ~90 of 220 catalog companies showing 0 PMs, with a mix of legit-zero (Block, Palantir, Wiz, xAI) and silently-broken (Confluent's Ashby team filter dropping 5 PMs, AmEx silently migrating Eightfold → Oracle HCM with 225 PMs invisible). 8-step fix plan held in `project_zero_jobs_audit_20260513` memory: `is_verified_zero` + `is_verified` columns, fix Ashby team filter at `scraper.ts:1071-1083`, DB swap AmEx to Oracle, per-company SPA scrapers (Apple/Tesla/Meta/TikTok/Wayfair), and a Claude Code scheduled-agent ground-truth check at 90% title-level threshold. See `feedback_silent_zero_failures.md` for the architectural principle.
