...
| 20. Specialized AI Agents and PR-Gated Deploys | Every task started with no context, and one careless change could hit production. | Built 13 reusable AI helpers that already know the project, and made every change go through a review step with a preview before it can go live. | Specialized helpers are worth it because each gives a predictable result you can check in seconds; most off-the-shelf ones need heavy trimming to fit a one-person project. |
| 21. Silent Zero Failures and Three New ATS Platforms in One Night | About 90 companies showed zero jobs, and I couldn't tell if they weren't hiring or the scraper had quietly broken. | Made the system flag every unexplained zero for me to confirm, and built or fixed scrapers for Apple, Shopify, eBay and others. | The dangerous bug is the one that looks like success, so a silent zero had to become something I'd actually see. And published references go stale, so I always re-check against the live site. |
| 22. Targeted Catalog Growth After The Common Crawl Detour | The catalog had stalled around 216 mostly-tech companies and I wanted it to feel like a real database. | Tried finding companies in bulk (turned up 1,352, but all names nobody recognizes), dropped it, and hand-added 29 recognizable banks, biotechs and consultancies instead. | Growing in the wrong direction is worse than not growing. And a scraper that runs isn't the same as one that returns useful jobs, so I check each against real results before counting it. |
| 24. Turning The Product Into A Content Engine (Weekly LinkedIn Digest) | The interesting trends in my data were invisible to anyone outside the product. | Turned the data into a ready-to-post weekly LinkedIn summary that sends itself every Friday. | Your own data is an edge when you lead with angles nobody else can see. And getting AI to write in your voice takes real guardrails, not a polite request. |
| 23. Job-First Feed, Comp-Filterable, Plus The Email-Disappearance Bug | The dashboard didn't scale, the daily email was getting noisy, and then I stopped getting my own email. | Built a feed that lists jobs across all companies with filters, cleaned up the email noise, and traced the missing email to a default that quietly stopped processing users once we passed 50. | Default settings are silent traps: everything worked until we crossed an invisible line. And when someone else sends your email for you, ask them what happened instead of guessing. |
...
| 29. Auditing The Whole System Before Charging For It | About to add payments and voice, I wanted the foundation sound, and the audit kept finding failures that stay silent. | Audited scrapers, security, login, cross-user access and speed; swept out queries that quietly returned partial results, locked down two publicly-readable tables, and rebuilt the daily email in layers so a drop can't hide. | The most dangerous failures are the quiet ones: a loud one gets fixed in an hour, a silent one runs for months looking fine. Put a different alarm at each layer. |
| 30. Doubling The Catalog And Making "Add" Actually Work | The catalog had stalled, adding a company showed nothing until the next day, and a few big employers showed nothing at all. | Grew the catalog from 247 to 519 off a public company-ranking list, added an "add a whole category" flow, decoupled scraping from the daily email so adds populate within minutes, and shipped a bought-data restore for the employers we can't scrape. | A catalog you can add to is worthless if adding doesn't show results. The real blocker wasn't a missing feature, it was an old assumption that scraping and emailing always happen together. |
| 31. The Night The Daily Job Died Silently | Right after I doubled the catalog, the daily run died partway through because I shipped a change that restarted the server mid-run, no email went out, and nothing told me. | Built a watchdog that runs outside the app and emails me if the run didn't finish, plus a start-heartbeat, an alert when a deploy kills the run, a resumable run, an enforced no-deploys window, and a monthly probe that proves the alarms themselves still work. | You have to monitor the monitors. The fix for a silent failure is a witness standing outside the thing that failed, not another check inside it. |
...
## Phase 27: Voice Mock Interviews

**Goal:** Help people get the job, not just find it.

I've spent years on both sides of the interview table, my own job hunt and then coaching other PMs through theirs, and the interview is almost always the part that actually decides it. The trouble is you can't really practice it alone, and there are only so many times you can ask a friend to put you through a mock. NewPMJobs already helps people find the openings, so the obvious next step was to give them a realistic mock interview they can run anytime.

**What I built:** a mock interview you can talk to out loud. You pick the type, a voice runs it like a real interviewer would, one question at a time and pushing for specifics, and afterward it scores you against a real rubric, in my voice rather than the usual AI cheerleading, and remembers your last session so the next one builds on it. It's just me using it for now, since the voice minutes cost real money and I want it good enough to charge for first.

The piece I actually care about isn't built yet: feedback on how you *sounded*, not just what you said, the thing a transcript can't give you but the audio can. I proved it works on one real recording before committing to the build, and that's exactly the part a transcript-only competitor can't copy.

**What I learned:** test the risky idea on one real example before you build everything around it. The voice and the scoring were never in doubt; whether a machine could honestly judge how I came across was, so that's the part I proved first.
...
## Phase 30: Doubling The Catalog And Making "Add" Actually Work

**Goal:** make the catalog feel like a real database, and make adding a company actually show its jobs.

Two problems had been nagging at me. The catalog had stalled around 247 companies, mostly the tech names everyone already knows. And worse, adding a company barely worked: when I added one, nothing showed up until the next day's scrape. The reason turned out to be a design knot I'd tied early without noticing. Scraping a company and emailing everyone about new jobs were the same job, welded together in the daily run. So there was no way to scrape a newly-added company on its own without also firing a daily email to every user, which I obviously couldn't do. As I put it at the time, it's a little crazy that I could add a company and then have to wait a full day to find out whether it had anything.

**What I built.**

First, growth. I found that a popular salary-comparison site publishes a public ranked list of tens of thousands of companies, and used that as a discovery source, then verified each candidate's job board actually returned real PM roles before counting it. The catalog roughly doubled, from 247 to 519. To make a bigger catalog usable, I built an "add by category" flow: instead of adding companies one at a time, you browse by industry and add a whole group at once, with the tech bucket split into sub-categories like AI, developer tools, and security.

Second, I cut the knot. I split scraping apart from emailing, so the system can now scrape any set of companies on demand without sending a single email. Then I wired that into the moment it matters: when you subscribe to a company that has no jobs yet, the system quietly scrapes it in the background, and its roles appear within a couple of minutes instead of the next day. The daily run is still the safety net that catches anything the instant scrape missed.

Third, the employers we can't scrape. A handful of big names block us outright, and rather than keep losing that fight, I'd decided to buy their listings from a jobs-data provider. I built that restore this session. It pulls those companies' real openings from the provider and quietly switches them back on, and it's written to turn itself on automatically once the provider's free quota resets, so there's no manual step waiting on me; until then it sits completely dormant. An independent review of my own code caught a real bug before it shipped, the kind that would only have surfaced in production weeks later.

**What I learned:** a catalog you can add to is worthless if adding doesn't show results. The thing blocking that wasn't a missing feature, it was an assumption baked into the architecture a long time ago, that scraping and emailing always happen together. The fix was to question the coupling, not add another button. And growth only counts if it's names people recognize and boards that actually return jobs, not a bigger number for its own sake.

---

## Phase 31: The Night The Daily Job Died Silently, And I Built The Alarm That Watches The Alarms

**Goal:** make it impossible for the daily job to fail without me knowing.

The day after I doubled the company catalog, the daily run that scrapes every company and emails everyone their new jobs simply did not happen, and nothing told me. I found out hours later by checking the database by hand. What happened: the bigger catalog made the run take far longer, and while it was still running I shipped an unrelated change, which restarted the server and killed the run partway through. No email went out. No alarm fired.

Why no alarm fired is the part worth sitting with. Every safeguard I had built lived inside the same job and ran at the very end of it: the "did the email volume drop" check, the status email to myself, all of it sat after the long scrape that died. And a server being killed isn't an error the code can catch, it just stops. So a job that never finished looked exactly like a quiet, healthy day. Broken, still running, and never started were all indistinguishable from the outside, and the health check stayed green the whole time because a fresh server had taken over fine.

**What I built.** I treated "the run finished" as something that has to be proven from outside the run. A watchdog now runs on a different system entirely, on its own schedule, and asks the app one question: did today's run actually complete? If the answer is no, or the app doesn't even answer, it emails me. The job also writes a heartbeat the moment it starts, not just when it ends, so a run that dies halfway leaves a visible "started but never finished" mark instead of nothing at all. If a deploy kills the run, the shutdown now fires a real alert naming the exact company it died on. I made the run resumable, so a restart picks up where it left off. And I made it an enforced rule that nobody can ship a change during the daily run's window.

While fixing this I found the same disease elsewhere: an email alert I thought was protecting me had been quietly dead for who knows how long, because a setting it needed was never configured, and a code review can't see a missing setting. So I added a monthly probe that actually sends a test alert and confirms it arrives, a dead-man's-switch for the alarms themselves.

**What I learned:** you have to monitor the monitors. Every guardrail I had checked some specific thing, but nothing checked whether the work even ran, or whether the alarms could still go off. The fix for a silent failure is never another check inside the thing that failed. It is a witness standing outside it. The most dangerous outage is the one that looks identical to everything being fine.
