...
| 27. A Voice That Interviews You Back | NewPMJobs finds people the job; it doesn't help them land it. The interview is the part people lose sleep over, and it's the natural next product for the same audience at the same moment. | A practice interview you can actually talk to: pick behavioral, product sense, or analytics, and a voice interviewer asks one question at a time, probes for specifics, and stays in character. Feedback comes after the call: the transcript is scored against a real rubric, written back in my voice, and it remembers you between sessions so it builds on last time instead of starting cold. Gated to just me for now, because the voice minutes cost real money. The headline bet, still being built, is feedback on HOW you sounded (pace, pauses, tone), which I validated on a real recording before writing any product code. | Validate the risky, unproven part first, on one real example, before you build the easy part around it. The defensible feature is the one the easy version structurally can't copy: a transcript can't tell you that you sounded nervous, but the audio can. And when the medium changes (a text coach becoming a voice interviewer), the same content needs a completely different shape. |
...
## Phase 27: A Voice That Interviews You Back

**Goal:** Help the people I track jobs for actually land the job, not just find it.

NewPMJobs tells a PM where the openings are. That's the front half of the problem. The back half is the interview, and it's the part people lose sleep over. I'd spent years on the other side of that table, and coaching PMs through it, so the natural next product was a mock interview you can actually talk to: a voice that asks you a real PM question, listens, pushes back, and then tells you how you did.

### What I Built

A practice interview that holds a live spoken conversation. You pick behavioral, product sense, or analytics, and a voice interviewer asks you one question at a time, probes the way a real interviewer does ("was that you or the team?", "so what actually happened?"), and stays in character. No coaching mid-call, no scoring out loud. Just the interview.

The feedback comes after. The transcript gets scored against a real rubric, one I built from my own coaching material, weighing the things an actual interviewer weighs: did you lead with the strategic picture, did you own the outcome, was the story specific or hand-wavy. And it's written back to you in my voice, not generic AI cheerleading.

Two things I'm proud of. First, it remembers you. After each session it writes a short read on how you did, and the next interview reads that first, so it builds on what you're good at and digs at last time's weak spot instead of starting cold. Second, I ran the same answer through three different AI graders side by side, the big three models, so I could see which one gives the most useful, honest feedback before I commit to one.

Right now it's gated to just me. The voice minutes cost real money, so I'm not opening it up until the experience is good enough to charge for.

### The Bet I'm Still Proving

The thing that makes this more than "another tool that grades your transcript" is feedback on *how you sounded*, not just what you said. Were you rushing? Did you go quiet for four seconds before answering? Did you sound confident or unsure? A transcript can't tell you that. Audio can.

I spent a session proving this was even possible before building it. I recorded a real practice answer and ran it through two engines: one that measures the hard numbers (your speaking pace, where the long pauses landed) and one that judges the tone (did you sound nervous, warm, confident). The proof it was actually working: both engines independently flagged the same two pauses, which means the tone engine was genuinely listening to the audio, not just rephrasing the transcript. It even caught my mistake for me. One engine flagged a twelve-second "pause" that turned out to be the interviewer talking, which told me exactly what to fix before building it for real: only count the silence while *you're* the one supposed to be talking.

The delivery report isn't in the product yet. The proof worked; the build is the next step. It's the part I think people will actually pay for.

### What I Learned

**A spoken interviewer and a text coach are different products.** My coaching material was written for a chat assistant that explains, demonstrates, and scores as it goes. I pictured pasting that into a voice agent and it falls apart instantly: a real interviewer doesn't break character to show you how to answer. The coaching had to move *after* the call. Same content, completely different shape, because the medium changed.

**Validate the hard part before you build the easy part.** The risky, unproven piece here was never the voice chat or the scoring. It was whether AI could give honest feedback on delivery from audio at all. So that's what I tested first, on one real recording, before writing a line of product code. It worked, and it surfaced the one real gotcha (the pause-counting bug) while it was still cheap to fix. The opposite order, build the whole feature and then find out the core bet doesn't hold, is how you waste a month.

**Build toward the moat, not the demo.** Anyone can ship a voice bot that grades a transcript. The defensible version is the one that does what the easy version structurally can't: tell you how you came across. When you're choosing what to build, build toward the part a competitor can't copy in a weekend.
