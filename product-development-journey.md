...
- **Batch three (five custom scrapers, one PR):** Goldman Sachs via their proprietary "Higher" GraphQL platform, EY via SAP SuccessFactors, KPMG via a bespoke WordPress backend, Klarna via the Deel job-board system, Revolut via their self-hosted Next.js data JSON. Plus BCG via the Phenom scraper I already had.
...
The backend got a new public read endpoint at `/api/feed`. Server-side filters for region, industry, level, city, company, minimum compensation. Per-job comp tier enrichment from the existing `comp_cache` table — read the company's early/mid/director TC ranges, map the job's classified level to the matching tier, surface as a string. Sort by comp also works.
...
- *Fourteen-day return rule.* A job that genuinely disappears and comes back deserves to count as new, but only if it's been gone at least two weeks. Added a `last_removed_at` timestamp on every job, stamped when status flips active→removed, and gated the "new" classification on it being at least fourteen days old.
...
- *Per-company seniority threshold.* New column on the companies table: `min_relevant_seniority` (early / mid / director). Manually set Google, Meta, Apple, Amazon, Netflix, Microsoft to "mid" so junior PM titles at those companies don't email the user. FAANG titles are genuinely inflated industry-wide; the threshold protects against that.
...
**The pagination bug.** When admin (me) stopped getting the daily email, the first place I looked was Resend. Zero record of even trying to send. So the cron loop wasn't iterating over my user record. I added some logging, watched the next run, and the root cause emerged: `supabase.auth.admin.listUsers()` defaults to a page size of fifty. We had crossed fifty total users the day before. The sixteen oldest users — including me, the first signup — were silently being paginated out of the daily iteration.

One-line unblock-fix went out within minutes: pass `{ perPage: 1000 }` at every call site. Next morning I followed up with a proper cursor-paginated helper (`listAllUsers`) and migrated all five call-sites to it. New rule: never call `supabase.auth.admin.listUsers` directly anywhere in this codebase. The helper enforces correct behavior for any future call-site too.
...
**Manually seed FAANG seniority thresholds instead of waiting for comp-validated automation.** The right long-term answer is to read each company's early-tier TC from levels.fyi and set `min_relevant_seniority` automatically — Google's $183k early-tier TC says their "PM" title is junior-by-industry. But that's a backlogged build. The user needed the FAANG noise filtered out of his email now. Manual seeding for the six FAANG companies took five minutes. The tradeoff is the manual decision can drift if Google rewrites their leveling guide, but FAANG title inflation has been stable for years; I'm comfortable revisiting if it changes.
...
**Default values in SDKs are landmines when the default looks reasonable until you cross an invisible boundary.** Pagination defaults are the classic example. `perPage=50` worked perfectly for the first fifty days of growth. The fifty-first user was the cliff. Nothing in the SDK's behavior changes at that boundary; the cliff is in your interpretation of the response. Going forward, any paginated SDK call gets explicit pagination handling, never the default. I've added this as a project convention so future me doesn't repeat it.
...
**PostgREST has multiple silent failure modes that look like working features.** Three found in this sprint: nested-table ordering silently no-ops with both `foreignTable` and `referencedTable` options; OR clauses use comma as delimiter so values containing commas need double-quotes or you get a malformed query; `ilike` vs `like` is case-sensitivity, which means a region abbreviation like ", NE" can match "Ne" in "New Jersey" and put New Jersey jobs in the Midwest bucket. Each was a one or two line fix once I knew about it. The cost was the half-day per bug spent learning the failure mode.
...
**Auto-trigger inside the existing daily cron.** The Railway daily cron at 14:00 UTC already runs the scraper, sends per-user alerts, refreshes comp data, and fires the admin digest. I embedded the weekly digest right after the admin digest with a `getUTCDay() === 5` gate. Friday only. No new Railway schedule needed. The email lands at ~14:25-14:35 UTC every Friday morning — 7:25am PT — exactly when LinkedIn engagement starts ramping for the day. The pattern is now the canonical way to add any future weekly task: embed in `runDailyCheck()`, gate by day, never add new Railway crons.
...
**Embed in the daily cron vs new Friday Railway cron.** I could have added a second Railway cron entry that fires only on Fridays. I didn't, because Railway cron is an operational surface that should be minimized. One canonical entry (`/api/cron/trigger`) with day-of-week gates inside the code is cleaner — fewer dashboard entries, fewer credentials to manage, the daily flow always runs first so the digest sees fresh data.
...
**The cron-day-of-week pattern is reusable infrastructure.** This is the third or fourth time I've reached for "fire this only on Fridays" or "fire this only on Mondays" inside the daily cron. Once it became clear this was a pattern, I documented it in the sidecar: embed inside `runDailyCheck()` with a UTC day gate plus a `forceXDigest` option. Never add new Railway cron entries. Any future weekly job (retention email, leaderboard, weekly report) follows the same pattern.
...
I spawned the incident-triage agent on the broken templates. It found the smoking gun within minutes. My deployed templates used `{{ .ConfirmationURL }}` — which is Supabase's default. That variable generates URLs pointing at Supabase's hosted verify endpoint (`<project>.supabase.co/auth/v1/verify?...&redirect_to=...`), NOT at our app's `/auth/confirm` route. Supabase's hosted endpoint validates the token (so `email_confirmed_at` gets set) then redirects the user to `redirect_to`, but the session cookie is never established because our SSR route never runs.

The documented format — sitting in our own `backend/src/middleware/AUTH.md` sidecar I should have read before deploying — was:

```
{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=magiclink
```

The sidecar enforcement hook is folder-based: editing files in `backend/src/middleware/` triggers AUTH.md. My deploy script lived in `backend/src/scripts/`, so the hook didn't fire. I trusted the hook to catch me when it couldn't.
...
I spawned a debugger agent on `/auth/confirm/route.ts`. It came back with a quote-this-verbatim finding:

> `NextResponse.redirect()` doesn't carry cookies set by `verifyOtp()`. The Supabase SSR client writes session cookies onto the Next.js `cookieStore` via the `setAll` callback, but `NextResponse.redirect()` constructs a fresh response and inherits none of them. The browser receives a 302 with an empty `Set-Cookie` header. Session is gone before the redirect is followed.

This bug had been there since `/auth/confirm` was introduced. Same class of bug PR #26 fixed in `middleware.ts` via `redirectPreservingSession()` — but never backported to the route handlers. Eleven users were in the "confirmed but no session" state before my template change had even happened.
...
- Re-PATCHed the Supabase Auth templates to the documented `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=...` format. The script lives at `backend/src/scripts/update-auth-templates.mjs` (gitignored) and is reusable for any future template change.
- Fixed the cookie-drop bug in both `/auth/confirm/route.ts` and `/auth/callback/route.ts` with six lines: explicitly `cookieStore.getAll().forEach(c => response.cookies.set(c))` onto every redirect. AUTH.md updated with the gotcha.
- Built `POST /api/admin/users/send-magic-link` (PR #63) so I could bulk-recover the 18 stuck users via a DevTools snippet, plus power any future "user is stuck, send them a fresh link" workflow. Endpoint uses `supabase.auth.signInWithOtp({ shouldCreateUser: false })` — recovery only, can't be misused to create accounts.

**Layer two — three independent defenses so this class of incident can't recur silently:**

- **DEV-12 [CI auth template check]** (PR #64) — GitHub Action that runs on every auth-touching PR plus nightly. Fetches the deployed Supabase Auth config via the Management API, asserts the template URL pattern matches AUTH.md. Fails the PR if `{{ .ConfirmationURL }}` is present or `{{ .TokenHash }}` is missing. Would have caught the original bug at PR time before merge. Requires `SUPABASE_PAT_CI` GitHub secret (pending Vik to add).
- **DEV-11 [daily code audit]** (PR #65) — heuristic GitHub Action cron at 15:00 UTC daily. Scans the last 24h diff for four risk patterns: `{{ .ConfirmationURL }}` in auth files (non-comment), `NextResponse.redirect` in route handlers without preceding `cookieStore.getAll()`, JWT-shaped strings in source, new backend routes without `requireAuth`. Emails admin only on findings. Heuristic skips `.github/ci/`, markdown, and comment lines so docs and the CI infra itself don't false-positive. Catches both yesterday's class of bug AND drift introduced via the Supabase Dashboard between PRs.
- **DEV-13 [PostHog auth funnel]** (PR #66) — four events instrument the auth flow: `auth.signin_email_sent` (client-side, after `signInWithOtp` returns success), `auth.signin_link_clicked` (server-side, at `/auth/confirm` entry), `auth.signin_success` (server-side, after `verifyOtp` succeeds + cookie copy), `auth.signin_failure` (server-side, on verify error). Server-side events post directly to PostHog's `/capture/` HTTP endpoint via a new `frontend/src/lib/serverAnalytics.ts` helper — no SDK dependency. A 50% drop-off alert between `link_clicked` and `success` would have paged me ~6 hours after the broken templates deployed.
...
**Cosmetic tasks aren't cosmetic when they touch load-bearing config.** Framing the email template change as "design polish" was the original sin. The variable name (`{{ .ConfirmationURL }}`) should have prompted "what does this actually generate?" but I treated Supabase's documented default as inherently safe. The fix isn't "read sidecars next time" — that's an honor system. It's automated: a CI check that fails the build if the deployed template doesn't match the documented format. The honor system is fine for behavior nobody actually performs; for behavior that gets performed regularly, automate the constraint.
...
**Mislabeled issues hide root causes for months.** DEV-3 was titled "Magic link fix — non-Gmail, Safari mobile" because that's what the symptoms looked like. The bug was universal — `NextResponse.redirect()` drops cookies — but Safari Mobile's slower session restoration made it more visible. If the title had been "auth route handlers drop session cookies on redirect," it would have been recognizable as a different class of bug and probably been fixed faster. Symptoms aren't root causes; titles based on symptoms encode the wrong mental model. Going forward, I'll title issues by the architectural artifact that's broken, not the user-visible failure.

**SDK defaults are landmines, again.** The Supabase Auth template default of `{{ .ConfirmationURL }}` works for projects that use Supabase's hosted verify flow. Our project uses the SSR `verifyOtp()` flow that needs `{{ .TokenHash }}`. The default LOOKS right (it's documented, it's what every example uses, it's what the dashboard suggests) but doesn't match our setup. Combined with Phase 23's `perPage=50` pagination gotcha, this is becoming a pattern: every SDK has invisible cliffs hidden behind reasonable-looking defaults. New rule: never use SDK defaults for anything load-bearing — explicit values only, every time, with a comment explaining why this value matters.
...
**Replaced the unstable ratio alert with a flat volume alert.** The PostHog alert was computing `auth.signin_success / auth.signin_email_sent` over a rolling 6-hour window, checked hourly, fires below 50%. The technically-correct measurement of conversion. The actual behavior on low traffic was statistical noise: a user who requests a magic link at 9am and clicks it at 4pm shows up in the success count at 4pm but with no matching email_sent in the same 6-hour lookback. The ratio routinely swung between 0% and 200%+ based on the timing of individual events, not based on whether auth was actually working. I disabled it, renamed it "RETIRED" so the historical chart stays available, and built a replacement: a new trends insight that counts `auth.signin_success` over the last 24h, displayed as a single number, with a daily alert that fires only if the count drops below 1. That catches catastrophic auth failure — template regression, cookie drop, the 2026-05-22 incident class — without misfiring on individual event timing. The funnel insight stays in PostHog as the diagnostic view: when the volume alert fires, the funnel shows where in the flow users dropped off. PostHog blocks alerts on Funnels queries directly even when set to trends visualization mode, so the trends-volume approach is the practical replacement.

**Added missing PostHog instrumentation to the PKCE auth-callback route.** While debugging the alert, I discovered that the `/auth/callback` route — which handles same-browser OAuth/code-exchange signins — was firing zero PostHog events. Only `/auth/confirm` (the cross-device token_hash flow) was instrumented. Any signin completing through callback was invisible to monitoring, which means the funnel was undercounting real signin volume. Mirrored the three-event pattern from confirm: `auth.signin_link_clicked` on entry, `auth.signin_success` on successful code exchange, `auth.signin_failure` on PKCE error. All tagged with `flow: "pkce"` so the funnel can split by route.
...
- **Auto-verify zero (7 days):** A company whose scraper has worked before (we've seen `is_verified=true`) and returns zero PMs for seven consecutive days auto-marks itself `is_verified_zero=true`. Trusted scraper plus sustained zero equals real zero. No human needed.
- **Auto-disable silent zero (14 days):** A company whose scraper has *never* returned a single PM (`is_verified=false`) and stays at zero for fourteen days auto-disables itself. Probably a misconfigured scraper. Saves cron compute; the existing Monday probe still gives it a chance to recover.
- **Auto-flip back:** Any scrape returning more than zero PMs resets `consecutive_zero_days=0` and, if `is_verified_zero` was true, flips it back to false. **There is no permanently-silenced state.** Every scrape re-evaluates from scratch. A company that goes Disney → 0 → 3 PMs → 0 again cycles correctly through silenced and unsilenced over the course of a year.

Bootstrapped all 28 currently-zero companies to silenced state via the same migration that added the `consecutive_zero_days` column. The migration's first cut was conservative (only silence is_verified=true companies), but when applied to prod, zero of the 28 matched — all 28 noise items were `is_verified=false`, mostly stealth-tier blocked sites like Meta, TikTok, Tesla, and Wayfair plus scrapers that have never returned a PM. Switched the bootstrap to catch-all. The auto-flip-back logic restores accuracy if any of them start hiring, so silencing was safe.

Deleted the email section, the rollup chip in the TL;DR header, the subject-line contribution ("28 unverified zeros"), the analysis-layer pattern detection that surfaced platform-level zero clusters (the admin no longer triages, so no human needs the pattern hint), and the `unverifiedZeros` field from the `AdminDigestInput` type and the `buildDigestAnalysis` function.
...
**Aggressive bootstrap vs conservative scope.** First cut of the migration scoped the bootstrap to `is_verified=true` companies only. Safer, more defensible: we trust the scraper, so we trust the zero. But it silenced zero of the actual noise items. The pragmatic version silences all currently-zero rows regardless of verification state, and trusts the auto-flip-back logic to restore accuracy when any of them start hiring. The safety argument is that `is_verified_zero` has zero user-facing blast radius — it's read only by the admin email plumbing and the cron itself — so worst-case bug is "admin email stays quieter than it should," never "users miss jobs." Knowing the blast radius is what unlocked the bolder default.
...
**Three rules in cron vs a separate background job.** Considered building this as a separate scheduled task that runs the auto-verify and auto-disable logic across the whole catalog independently from the daily scrape. Rejected as added complexity. The signals these rules act on — `is_verified`, `is_verified_zero`, `total_product_jobs`, scrape success/failure — are all updated by the daily scrape itself. Putting the logic inline with the per-company update means we evaluate using the freshest possible state, and we only need one cron entry. Same reasoning as the weekly LinkedIn digest pattern: embed in `runDailyCheck()`, don't add Railway cron entries.

### What I Learned

**The flag's blast radius determines whether bold defaults are safe.** I greped the entire codebase for `is_verified_zero` before making the bootstrap aggressive. It's read in four files, all backend, all admin-email plumbing or cron logic. No user-facing surface — the public feed, daily user alerts, dashboard, and `seen_jobs` writes are all blind to it. That bounded downside is what made the aggressive bootstrap safe. If the flag had been used to suppress jobs from user-facing views, the conservative version would have been the only defensible call. The lesson generalizes: before changing the default behavior of a flag, audit its blast radius. The semantics are only safe to relax if you know what's reading the flag.
...
**When the data evolves, the workflow has to evolve with it.** The "Unverified zeros" email was the right answer at the time it shipped. The system genuinely couldn't distinguish "this company has no open PMs" from "this scraper is broken and returning zero." Adding the `is_verified` column gave the system that signal — but the email kept asking the human anyway, out of inertia. The structural failure isn't that the email was bad, it's that the email outlived its information environment by months. Every workflow that asks a human to do something the system could do is an unpaid tech debt. Find them by looking at recurring asks in your own inbox.
...
| 57 | Cloudflare bypass via static-data JSON routes when HTML is bot-challenged | Revolut Next.js /_next/data path (Phase 22) |
| 58 | Auto-refresh of rotating identifiers via stealth-tier intercept + inferPlatformFromSniffedUrl | Revolut buildId pattern (Phase 22) |
...
| 63 | SDK default values are invisible cliffs — never trust paginated defaults | Supabase listUsers perPage=50 silently skipped admin (Phase 23) |
...
| 70 | Cron-day-of-week gate inside `runDailyCheck()` is the canonical pattern for any weekly task — never add new Railway cron entries | Friday auto-trigger pattern embedded in daily cron (Phase 24) |
...
| 72 | Editorial structure should be locked in code once approved — `renderLinkedInPost()` is the canonical post template, future edits need re-approval | Weekly digest post structure freeze (Phase 24) |
