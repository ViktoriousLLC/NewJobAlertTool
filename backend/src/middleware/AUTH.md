...
- **NextResponse.redirect() drops cookies set by Supabase SSR**: `verifyOtp()` / `exchangeCodeForSession()` write session cookies onto the Next.js `cookieStore` via the `setAll` callback, but `NextResponse.redirect()` constructs a fresh response and inherits NONE of those cookies. Must copy explicitly with `cookieStore.getAll().forEach(c => response.cookies.set(c))`. This is the same class of bug `redirectPreservingSession()` fixes in middleware.ts. Bit us 2026-05-20 onwards in `/auth/confirm` and `/auth/callback` route handlers — caused 11+ "confirmed but no session" users. Fixed 2026-05-25.
