...
### 2026-05-14
- **Silent zero failure class (closed)**: A 0-PM scrape with `last_check_status='success'` looks identical to "genuinely 0 PMs" and "scraper silently broken." Phase 1 added `is_verified` + `is_verified_zero` columns. Every 0-PM company appears in daily admin-digest "Unverified zeros" section until admin marks `is_verified_zero=true` OR scraper recovers (is_verified flips true on next >0 result). **NEVER mark `is_verified_zero=true` just to clear the email** — always confirm legit-zero first.
