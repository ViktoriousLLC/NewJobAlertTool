...
//   * AUTO_DISABLE_THRESHOLD: consecutive failures before a company is
//     auto-disabled and skipped until the Monday watch-list probe.
//   * AUTO_VERIFY_ZERO_DAYS: a previously-verified scraper at 0 PMs this long
//     gets auto-marked is_verified_zero=true (trusted scraper + sustained zero).
//   * SILENT_ZERO_DISABLE_DAYS: a never-verified company at 0 this long gets
//     auto-disabled (scraper is probably broken from day one).
//   * PROBE_DAY_OF_WEEK: Monday (UTC) is when auto-disabled companies get one
//     probe attempt.
...
// Per-run accumulators the daily admin digest reads. The scrape-on-demand path
// passes a throwaway context (it never builds a digest), so the same scrape +
// seen_jobs logic runs with zero email/digest coupling.
interface ScrapeContext {
  ...
  // Monday-UTC probe day: auto-disabled companies get one re-attempt. The
  // scrape-on-demand path forces this to true so a freshly added (or manually
  // targeted) company is always actually scraped, never skipped as disabled.
  isProbeDay: boolean;
}

export function createScrapeContext(isProbeDay: boolean): ScrapeContext {
  return {
    companyAlerts: new Map(),
    failedCompanies: [],
    autoRemediated: [],
    stealthRecovered: [],
    autoDisabled: [],
...
// Outcome of a single company scrape, returned for the scrape-on-demand summary.
export interface PerCompanyScrapeResult {
  id: string;
  name: string;
  status: string; // last_check_status written this run, e.g. "success" / "error: ..."
  jobsAdded: number; // brand-new active jobs INSERTed this run
  totalActive: number; // active job count after the diff
  error?: string;
}
...
// the scrape-on-demand endpoint can reuse it WITHOUT touching any email or
// digest step. It mutates `ctx` accumulators (which the daily digest reads) and
// returns the per-company outcome. Never throws — scrape failures are caught,
// recorded on the company row + ctx.failedCompanies, and returned with an error.
export async function scrapeAndRecordCompany(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  company: any,
  ctx: ScrapeContext
): Promise<PerCompanyScrapeResult> {
  const isProbeDay = ctx.isProbeDay;
  ...
  if (company.auto_disabled && !isProbing) {
    ...
    return { id: company.id, name: company.name, status: company.last_check_status || "skipped (auto-disabled)", jobsAdded: 0, totalActive: company.total_product_jobs ?? 0 };
  }
  if (isProbing) {
  ...
  try {
      ...
      try {
        ...
        if (discovery && discovery.platformType !== company.platform_type) {
          ...
          if (rawJobs.length > 0) {
            ...
            ctx.autoRemediated.push({ name: company.name, from: prevPlatform, to: discovery.platformType });
      ...
      if (!isTransientFailure) {
        ...
        try {
          ...
          if (stealthResult.jobs.length > 0) {
            ...
            ctx.stealthRecovered.push({ name: company.name, jobCount: stealthResult.jobs.length });
            ...
            if (autoFixApplied) {
              ctx.autoRemediated.push({ name: company.name, from: autoFixApplied.from, to: autoFixApplied.to });
              await logScraperEvent(company.id, company.name, "auto_remediation", {
    ...
    if (isProbing && jobs.length > 0) {
      ctx.reEnabled.push({ name: company.name, jobCount: jobs.length });
    ...
    ctx.companyAlerts.set(company.id, {
      companyName: company.name,
    ...
    ctx.qualityData.set(company.id, {
      companyName: company.name,
    ...
    return {
      ...
      name: company.name,
      status: checkStatus,
      jobsAdded: toInsert.length,
      totalActive: currentJobCount,
    };
  } catch (err) {
    ...
    if (isProbing) {
      ...
      if (shouldAutoDisable) {
        ctx.autoDisabled.push({ name: company.name, reason: errMsg });
        await logScraperEvent(company.id, company.name, "auto_disabled", {
          reason: errMsg,
    ...
    ctx.failedCompanies.push({ name: company.name, error: errMsg, consecutiveFailures: newFailureCount });
    ...
    return {
      ...
      status: `error: ${errMsg}`,
      jobsAdded: 0,
      totalActive: company.total_product_jobs ?? 0,
      error: errMsg,
    };
  }
}
...
async function runDailyCheckInner(options?: { skipEmails?: boolean; forceMondayDigest?: boolean; forceWeeklyDigest?: boolean }): Promise<void> {
  ...
  // builds the digest/emails afterward exactly as before.
  const ctx = createScrapeContext(new Date().getUTCDay() === PROBE_DAY_OF_WEEK);
