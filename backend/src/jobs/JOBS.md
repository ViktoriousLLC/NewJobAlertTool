...
- **Resumable run**: the daily loop skips companies already scraped today (`last_checked_at` date = today's UTC date) so a re-triggered run resumes instead of restarting at company 1. `runDailyCheck({ force: true })` bypasses it for a deliberate full re-scrape. No-op on a normal 14:00 run (nothing checked that day yet).
