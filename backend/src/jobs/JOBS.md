...
- **Within-response de-dup**: the feed can surface one listing under multiple derived cities (or tracking-param variants that strip to the same path), so the module de-dups by `job_url_path` before the batch insert — otherwise the `(company_id, job_url_path)` UNIQUE constraint would fail the whole company's insert.
- **Insert/refresh only, never removes**: it only inserts new, re-activates removed/archived, and refreshes title/location. The 7-day LinkedIn window means older still-live roles fall out of the response; they age out via the existing 60-day archive, never delisted here.
- **Restored jobs surface in the FEED, not the restore-day daily email.** The pull writes straight to `seen_jobs`; it does not push into the daily-email `companyAlerts` accumulator. So on the day an employer is restored, its (potentially 8-22) roles appear on the site immediately but are not blasted to that company's subscribers as "new" — treated like a catalog backfill, which is what it is. (If restore-day alerting is ever wanted, thread the inserted jobs into `ctx.companyAlerts`.)
- **The daily ATS loop skips `platform_type='rapidapi_linkedin'` companies** (guard at the top of the `runDailyCheckInner` per-company loop). They're owned by this pull, and their `careers_url` is still the hard-blocked host, so letting the ATS loop touch them would stamp a misleading `success (0 jobs from source)` that the session-start health check flags.
...
- **Worker entrypoint** (`backend/src/jobs/dailyEntry.ts`, `npm run cron:daily`, DEV-57): a standalone process that runs the daily check once and exits — ready to move the run OFF the web service onto a dedicated Railway cron service so a web-service deploy can't kill it. The Railway service **cutover** is **DEV-58** (a dashboard flip that needs a real 14:00 run to verify; not done blind).
- **Email-only recovery** (`POST /api/cron/email-only`, `sendEmailOnlyFromToday()`, DEV-58): re-sends the daily alert from already-scraped `seen_jobs` (today's new active jobs, grouped into the same `companyAlerts` shape `sendPerUserAlerts` consumes) WITHOUT re-scraping — for "scrape finished but the email step failed/was skipped." **SAFE BY DEFAULT**: dry-run (returns what it *would* send, sends nothing) unless `?dryRun=false`; a real send refuses if today's daily run already emailed (`?force=true` overrides), and no-ops if there are zero new jobs today (so a quiet-day send can't fire Monday weekly digests). Mirrors the weekly path's build-from-DB pattern. **`force=true` re-runs the FULL send engine** (per-user recommendations + Monday weekly digests + a second `recommendation_history` write), not just a re-delivery of today's jobs — recovery-only, not routine.
