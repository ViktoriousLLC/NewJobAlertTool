...
- **Email-only recovery** (`POST /api/cron/email-only`, `sendEmailOnlyFromToday()`, DEV-58): re-sends the daily alert from already-scraped `seen_jobs` (today's new active jobs, grouped into the same `companyAlerts` shape `sendPerUserAlerts` consumes) WITHOUT re-scraping — for "scrape finished but the email step failed/was skipped." **SAFE BY DEFAULT**: dry-run (returns what it *would* send, sends nothing) unless `?dryRun=false`; a real send refuses if today's daily run already emailed (`?force=true` overrides). Mirrors the weekly path's build-from-DB pattern.
